...
class StackMate::CloudStackResource
  ...
  def set_metadata
    begin
      ...
      logger.debug("Adding metadata for resource " + workitem.participant_name)

      ActiveRecord::Base.connection_pool.with_connection do
        stack = Stack.find(@opts['stack_id'])
        resource = stack.stack_resources.find_by_logical_id(workitem.participant_name)
        json_metadata = resolved_metadata.to_json
        resource.metadata = json_metadata
        resource.save
      end
    rescue Exception => e
      logger.error("Unable to save metadata due to " + e.message)
      raise e
    end
  end
      ...
      ActiveRecord::Base.connection_pool.with_connection do
        ...
        if "create".eql?(work_item["params"]["operation"])
          ...
          if(!resource.physical_id.nil?)
            if(!"true".eql?(work_item[work_item.participant_name]['delete_error'].to_s))
